...
def add_tournament_form():
    ...
    while True:
        try:
            name = validate_tournament_name(input("🏆 Nom du tournoi : ").strip())
            break
    ...
    while True:
        try:
            end_date = validate_date(input("📅 Date de fin (JJ/MM/AAAA) : ").strip())
            start_date, end_date = validate_tournament_dates(start_date, end_date)
            break
    ...
    while True:
        try:
            rounds_total_input = input(
                "🔢 Nombre total de rounds (par défaut : 4) : "
            ).strip()
